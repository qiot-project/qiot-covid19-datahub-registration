package org.qiot.covid19.datahub.registration.service.impl;
...
import org.qiot.covid19.datahub.registration.certmanager.api.model.CertificateKeystoresSpec;
import org.qiot.covid19.datahub.registration.certmanager.api.model.CertificateSpec;
import org.qiot.covid19.datahub.registration.certmanager.api.model.KeystoreSpec;
import org.qiot.covid19.datahub.registration.certmanager.api.model.ObjectReferenceSpec;
import org.qiot.covid19.datahub.registration.certmanager.api.model.PasswordSecretRefSpec;
...
public class CertManagerCertificateService implements CertificateService {
	...
	public CertManagerCertificateService(CertificateOperation certificateOperation, SecretOperation secretOperation,
			Logger log, @ConfigProperty(name = "qiot.cert-manager.issuer") String issuer,
	...
	public RegisterResponse provision(RegisterRequest data) {
		...
		final String commonName = name + "." + certificateOperation.getNamespace() + domain;
		
		final Certificate certificate = Certificate.builder()
				.metadata(
					new ObjectMetaBuilder()
						.withName(name)
						.withLabels(Collections.singletonMap(REGISTRATION_QIOT_IO_SERIAL, data.getSerial()))
					.build()
				)
				.spec(
					CertificateSpec.builder()
						...
						.issuerRef(
							ObjectReferenceSpec.builder()
								.name(issuer)
							.build()
						)
						.keystores(createKeyStoreSecret(data))
					.build())
				.build();
	...
	private CertificateKeystoresSpec createKeyStoreSecret(RegisterRequest data) {
		...
		if (keyStorePassword != null && !"".equals(keyStorePassword)) {
			...
			secretOperation.operation().create(new SecretBuilder()
				.withNewMetadata()
					...
					.withLabels(Collections.singletonMap(REGISTRATION_QIOT_IO_SERIAL, data.getSerial()))
				.endMetadata()
				.withStringData(Collections.singletonMap(KEYSTORE_KEY_PASSWORD, keyStorePassword))
				.build()
			);

			return CertificateKeystoresSpec.builder()
					.pkcs12(KeystoreSpec.builder()
							.create(true)
							.passwordSecretRef(
								PasswordSecretRefSpec.builder()
		...
		throw new IllegalArgumentException("KeyStorePassword is undedifined for serial: " + data.getSerial() + ", name: " + data.getName());
	}
